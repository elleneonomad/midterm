...
public class RangingSearch {
    public static void main(String[] args) {
        List<ProductPost> list = Dataset.productList;
        // List<ProductPost> filtered = list.stream()
        // .filter(item -> item.getScore() >= 9.0)
        // .sorted((x, y) -> y.getScore().compareTo(x.getScore())) //sort reversed
        // .toList();
        // filtered.stream().forEach(x -> System.out.println(x));

        String keyword = "hoME";
        // find any item that its name or/and its description contains the keyword
        List<ProductPost> filtered = list.stream()
